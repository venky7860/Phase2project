...
/**
 * Servlet implementation class FlightListCtl
 */
@WebServlet(name = "FlightListCtl", urlPatterns = { "/ctl/FlightListCtl" })
public class FlightListCtl extends BaseCtl {
	...
	protected BaseBean populateBean(HttpServletRequest request) {
		log.debug("FlightListCtl populateBean method start");
		...
		log.debug("FlightListCtl populateBean method end");
		return bean;
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		log.debug("FlightListCtl doGet method end");
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (OP_SEARCH.equalsIgnoreCase(op) || OP_NEXT.equalsIgnoreCase(op) || OP_PREVIOUS.equalsIgnoreCase(op)) {
			...
			if (ids != null && ids.length > 0) {
				FlightBean deletebean = new FlightBean();
				for (String id : ids) {
		...
		log.debug("FlightListCtl doPost method end");
	}

	@Override
	protected String getView() {
		// TODO Auto-generated method stub
		return ATBView.FLIGHT_LIST_VIEW;
	}
